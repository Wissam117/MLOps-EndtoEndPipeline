...
if PY38_PLUS:
    def _is_singleton(node):  # type: (ast.AST) -> bool
        return (
            ...
            and isinstance(node.value, (bool, type(Ellipsis), type(None)))
        )
else:
    def _is_singleton(node):  # type: (ast.AST) -> bool
...
def _is_tuple_constant(node):  # type: (ast.AST) -> bool
    return (
        ...
        and all(_is_constant(elt) for elt in node.elts)
    )
...
def _is_typing_helper(node, is_name_match_fn, scope_stack):
    ...
    return (
        (
            ...
            and _bare_name_is_attr(node.id)
        ) or (
...
def is_typing_overload(value, scope_stack):
    return (
        isinstance(value.source, (ast.FunctionDef, ast.AsyncFunctionDef))
        and any(
...
def _collect_type_comments(tree, tokens):
    ...
    for tp, text, start, _, _ in tokens:
        if (
                tp != tokenize.COMMENT  # skip non comments
                or not TYPE_COMMENT_RE.match(text)  # skip non-type comments
                or TYPE_IGNORE_RE.match(text)  # skip ignores
        ):
            continue
...
class Checker:
    ...
    def addBinding(self, node, value):
        ...
        if value.name not in self.scope or not isinstance(value, Annotation):
            ...
            while (
                isinstance(value, NamedExprAssignment)
                and isinstance(self.scopeStack[cur_scope_pos], GeneratorScope)
            ):
                cur_scope_pos -= 1
        ...
        for scope in self.scopeStack[-1::-1]:
            ...
            if name == 'print' and isinstance(binding, Builtin):
                ...
                if (isinstance(parent, ast.BinOp)
                        and isinstance(parent.op, ast.RShift)):
    ...
    def CALL(self, node):
        ...
        if (
            ...
            if (
                ...
                and all(isinstance(x, (ast.Tuple, ast.List))
                        and len(x.elts) == 2 for x in node.args[1].elts)
            ):
                omit += ["args"]
    ...
    def _handle_percent_format(self, node):
        ...
        if (
                isinstance(node.right, ast.Dict)
                and all(isinstance(k, ast.Str) for k in node.right.keys)
        ):
