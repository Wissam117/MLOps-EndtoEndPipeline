        ...
        for attrib in dir(a):
            ...
            if isinstance(f, collections.abc.Callable):
                ...
                if attrib in methodargs:
                    ...
                    args = ()
                b = f(*args)
                assert_(isinstance(b, matrix), "%s" % attrib)
        assert_(isinstance(a.real, matrix))
        assert_(isinstance(a.imag, matrix))
        c, d = matrix([0.0]).nonzero()
        assert_(isinstance(c, np.ndarray))
        assert_(isinstance(d, np.ndarray))
...
class TestPower:
    def test_returntype(self):
        a = np.array([[0, 1], [0, 0]])
        assert_(isinstance(matrix_power(a, 2), np.ndarray))
        a = mat(a)
        assert_(isinstance(matrix_power(a, 2), matrix))
