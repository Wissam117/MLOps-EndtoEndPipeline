...
class TestSubclasses:
    def test_basic(self):
        ...
        s = a.view(SubClass)
        s_slice = s[:3]
        assert_(isinstance(s_slice, SubClass))
        assert_(s_slice.base is s)
        assert_array_equal(s_slice, a[:3])

        s_fancy = s[[0, 1, 2]]
        assert_(isinstance(s_fancy, SubClass))
        assert_(s_fancy.base is not s)
        assert_(isinstance(s_fancy.base, np.ndarray))
        assert_array_equal(s_fancy, a[[0, 1, 2]])
        assert_array_equal(s_fancy.base, a[[0, 1, 2]])

        s_bool = s[s > 0]
        assert_(isinstance(s_bool, SubClass))
        assert_(s_bool.base is not s)
        assert_(isinstance(s_bool.base, np.ndarray))
