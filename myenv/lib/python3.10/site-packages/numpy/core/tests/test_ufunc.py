...
class TestUfunc:
                ...
                for s1 in slice_3:
                    for s2 in slice_3:
                        ...
                        if (a1.shape[-1] == a2.shape[-2]
                                and broadcastable(a1.shape[0], a2.shape[0])):
    ...
    def test_scalar_reduction(self):
        ...
        assert_(isinstance(np.prod(np.float32(2.5), axis=0), np.float32))
        assert_(isinstance(np.sum(np.float32(2.5), axis=0), np.float32))
        assert_(isinstance(np.max(np.float32(2.5), axis=0), np.float32))
        assert_(isinstance(np.min(np.float32(2.5), axis=0), np.float32))

        # check if scalars/0-d arrays get cast
        assert_(isinstance(np.any(0, axis=0), np.bool_))

        # assert that 0-d arrays get wrapped
        class MyArray(np.ndarray):
            pass
        a = np.array(1).view(MyArray)
        assert_(isinstance(np.any(a), MyArray))
    ...
    def test_reduce_output_subclass_ok(self):
        ...
        assert isinstance(np.add.reduce(out), MyArr)
