...
class TestResize:
    ...
    def test_subclass(self):
        ...
        assert isinstance(np.resize(my_arr, 5), MyArray)
        assert isinstance(np.resize(my_arr, 0), MyArray)
        ...
        for view in (C, np.ndarray):
            for nd in range(1, 4):
                ...
                for nzx in (np.nonzero(x), x.nonzero()):
                    for nzx_i in nzx:
                        assert_(isinstance(nzx_i, np.ndarray))
...
def assert_array_strict_equal(x, y):
    ...
    if ((x.dtype.alignment <= 8
            or np.intp().dtype.itemsize != 4)
...
class TestAllclose:
    ...
    def test_return_class_is_ndarray(self):
        ...
        a = Foo([1])
        assert_(isinstance(np.allclose(a, a), bool))
...
class TestIsclose:
    ...
    def test_masked_arrays(self):
        ...
        assert_(isinstance(x, type(np.isclose(2, x))))
        assert_(isinstance(x, type(np.isclose(x, 2))))

        x = np.ma.masked_where([True, True, False], [np.nan, np.inf, np.nan])
        assert_(isinstance(x, type(np.isclose(np.inf, x))))
        assert_(isinstance(x, type(np.isclose(x, np.inf))))

        x = np.ma.masked_where([True, True, False], [np.nan, np.nan, np.nan])
        y = np.isclose(np.nan, x, equal_nan=True)
        assert_(isinstance(x, type(y)))
    ...
    def test_non_finite_scalar(self):
        ...
        assert_(isinstance(np.isclose(0, np.inf), np.bool_))
...
class TestLikeFuncs:
    ...
    def check_like_function(self, like_function, value, fill_value=False):
        ...
        assert_(isinstance(b, MyNDArray))

        b = like_function(a, subok=False, **fill_kwarg)
        assert_(not isinstance(b, MyNDArray))
