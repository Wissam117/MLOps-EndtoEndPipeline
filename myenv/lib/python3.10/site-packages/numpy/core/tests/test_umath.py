...
class TestFPClass:
    ...
    def test_fpclass(self, stride):
        arr_f64 = np.array([np.nan, -np.nan, np.inf, -np.inf, -1.0, 1.0,
                           - 0.0, 0.0, 2.2251e-308, -2.2251e-308], dtype='d')
        arr_f32 = np.array([np.nan, -np.nan, np.inf, -np.inf, -1.0, 1.0,
                           - 0.0, 0.0, 1.4013e-045, -1.4013e-045], dtype='f')
...
class TestLDExp:
    ...
    def test_ldexp(self, dtype, stride):
        ...
        assert_equal(np.ldexp(mant[::stride], exp[::stride],
...
class TestBitwiseUFuncs:
    ...
    def test_reduction(self):
        ...
        for f in binary_funcs:
            ...
            assert_(isinstance(f.reduce(btype), bool), msg)
...
class TestSpecialMethods:
    ...
    def test_priority(self):
        ...
        c = C()
        f = ncu.minimum
        assert_(isinstance(f(x, x), np.ndarray))
        assert_(isinstance(f(x, a), A))
        assert_(isinstance(f(x, b), B))
        assert_(isinstance(f(x, c), C))
        assert_(isinstance(f(a, x), A))
        assert_(isinstance(f(b, x), B))
        assert_(isinstance(f(c, x), C))

        assert_(isinstance(f(a, a), A))
        assert_(isinstance(f(a, b), B))
        assert_(isinstance(f(b, a), B))
        assert_(isinstance(f(b, b), B))
        assert_(isinstance(f(b, c), C))
        assert_(isinstance(f(c, b), C))
        assert_(isinstance(f(c, c), C))
...
def test_outer_bad_subclass():
    ...
    for cls in [BadArr1, BadArr2]:
        ...
        arr = np.ones((2, 3)).view(cls)
        assert isinstance(np.add.outer([1, 2], arr), cls)
