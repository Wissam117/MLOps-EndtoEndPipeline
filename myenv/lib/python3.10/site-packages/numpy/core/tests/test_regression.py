...
class TestRegression:
    ...
    def test_squeeze_type(self):
        ...
        a = np.array([3])
        b = np.array(3)
        assert_(isinstance(a.squeeze(), np.ndarray))
        assert_(isinstance(b.squeeze(), np.ndarray))
    ...
    def test_attributes(self):
        ...
        assert_(isinstance(dat.nonzero()[0], np.ndarray))
        assert_(isinstance(dat.nonzero()[1], np.ndarray))
    ...
    def test_for_object_scalar_creation(self):
        ...
        d = np.object_([None, {}, []])
        assert_(a is None)
        assert_(isinstance(b, int))
        assert_(isinstance(b2, float))
        assert_(isinstance(c, np.ndarray))
    ...
    def test_void_compare_segfault(self):
        ...
        a = sorted(np.ones(3, dtype=[('object', 'O'), ('int', '<i2')]))
