...
def _extendLine_pretty(s, line, word, line_width, next_line_prefix, legacy):
    ...
    if (len(line) + max_word_length > line_width
            and len(line) > len(next_line_prefix)):
        s += line.rstrip() + '\n'
    ...
    def fillFormat(self, data):
        ...
        if len(abs_non_zero) != 0:
            ...
            with errstate(over='ignore'):  # division can overflow
                if max_val >= 1.e8 or (not self.suppress_small
                                       and (min_val < 0.0001 or max_val / min_val > 1000.)):
...
def _array_repr_implementation(
    ...
    if (_format_options['legacy'] <= 113
            and arr.shape == () and not arr.dtype.names):
        lst = repr(arr.item())
...
def _array_str_implementation(
    ...
    if (_format_options['legacy'] <= 113
            and a.shape == () and not a.dtype.names):
        return str(a.item())
